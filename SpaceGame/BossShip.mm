...
-(id)initWithWorld:(b2World *)world layer:(ActionLayer *)layer {
    if ((self=[super initWithSpriteFrameName:@"Boss_ship.png"
                                       world:world
                                   shapeName:@"Boss_ship"
                                       maxHp:50
...
-(void)updateWithShipPosition:(CGPoint)shipPosition {
    ...
    if (!_initialMove) {
        _initialMove = YES;
        CGPoint midScreen = ccp(winSize.width/2, winSize.height/2);
        [self runAction:[CCMoveTo actionWithDuration:4.0 position:midScreen]];
    }
...
-(void)randomAction {
    ...
    if (randomAction == 0 || !_initialMove) {
        ...
        [_layer shootEnemyLaserFromPosition:[self convertToWorldSpace:_shooter1.position]];
        [_layer shootEnemyLaserFromPosition:[self convertToWorldSpace:_shooter2.position]];
        action = [CCDelayTime actionWithDuration:0.2];
    }
    else if (randomAction == 4) {
        CGPoint cannonHeadWorld = [self convertToWorldSpace:ccp(_cannon.position.x - _cannon.contentSize.width/2, _cannon.position.y)];
        [_layer shootCannonBallAtShipFromPosition:cannonHeadWorld];
        action = [CCDelayTime actionWithDuration:0.2];
    }
